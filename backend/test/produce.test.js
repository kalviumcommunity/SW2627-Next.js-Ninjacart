...
async function runProduceTests() {
  ...
  try {
    ...
    // 4. Create produce with 0 quantity sets status to SOLD_OUT
    ...
    const createSoldOutValid = createSoldOutRes.status === 201 && createSoldOutRes.body.data.status === 'SOLD_OUT';
    recordTest('6. Create Produce with 0 quantity sets status to SOLD_OUT', createSoldOutValid, JSON.stringify(createSoldOutRes.body));
    ...
    // 6. Update produce to 0 quantity changes status to SOLD_OUT
    const updateToZeroRes = await request('PATCH', `/api/produce/${produceId}`, { quantity: 0 }, {
      Authorization: `Bearer ${farmerToken}`
    });
    const updateToZeroValid = updateToZeroRes.status === 200 && updateToZeroRes.body.data.status === 'SOLD_OUT';
    recordTest('8. Update Produce to 0 quantity changes status to SOLD_OUT', updateToZeroValid, JSON.stringify(updateToZeroRes.body));
